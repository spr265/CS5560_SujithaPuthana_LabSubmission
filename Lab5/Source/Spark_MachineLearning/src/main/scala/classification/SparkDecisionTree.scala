...
object SparkDecisionTree {
  ...
  private def run(params: Params) {
    ...
    val IMAGE_CATEGORIES = List("WikiRef150", "WikiRef186", "WikiRef220")
    var index = 0
...
*/
    val df1 = sc.wholeTextFiles(paths.mkString(",")).map(f => {
      val wordSplit= WordSplit.returnword(f._2);
    ...
    dfseq.saveAsTextFile("DecisionTree/NONLPOUTPUT")
    ...
    tf.cache()
    //tf.saveAsTextFile("DecisionTree/tf")
    ...
    tfidf.saveAsTextFile("Decisiontree/TfidfNoNlp")
